from collections import defaultdict
import cmath

data = list(map(int, open("input.txt").readline().split(",")))


def program(data):
    r = defaultdict(lambda: 0)
    for key in range(len(data)):
        r[key] = data[key]
    position = 0
    rbase = 0

    def calculateParams(instruction, r, position, number):
        params = []
        for i in range(number):
            p = 0
            if instruction[-3 - i] == "0":
                p = r[position + 1 + i]
            elif instruction[-3 - i] == "1":
                p = position + 1 + i
            else:
                p = r[position + 1 + i] + rbase
            params.append(p)
        return params

    while r[position] != 99:
        instruction = "0000" + str(r[position])
        if instruction[-1] == "1":
            params = calculateParams(instruction, r, position, 3)
            r[params[2]] = r[params[0]] + r[params[1]]
            position += 4
            continue
        if instruction[-1] == "2":
            params = calculateParams(instruction, r, position, 3)
            r[params[2]] = r[params[0]] * r[params[1]]
            position += 4
            continue
        if instruction[-1] == "3":
            params = calculateParams(instruction, r, position, 1)
            inp = yield
            r[params[0]] = inp
            position += 2
            continue
        if instruction[-1] == "4":
            params = calculateParams(instruction, r, position, 1)
            position += 2
            yield r[params[0]]
            continue
        if instruction[-1] == "5":
            params = calculateParams(instruction, r, position, 2)
            if r[params[0]] != 0:
                position = r[params[1]]
            else:
                position += 3
            continue
        if instruction[-1] == "6":
            params = calculateParams(instruction, r, position, 2)
            if r[params[0]] == 0:
                position = r[params[1]]
            else:
                position += 3
            continue
        if instruction[-1] == "7":
            params = calculateParams(instruction, r, position, 3)
            r[params[2]] = 1 if r[params[0]] < r[params[1]] else 0
            position += 4
            continue
        if instruction[-1] == "8":
            params = calculateParams(instruction, r, position, 3)
            r[params[2]] = 1 if r[params[0]] == r[params[1]] else 0
            position += 4
            continue
        if instruction[-1] == "9":
            params = calculateParams(instruction, r, position, 1)
            rbase += r[params[0]]
            position += 2
            continue
        print("error")


total = 0
for y in range(50):
    for x in range(50):
        p = program(data)
        next(p)
        p.send(x)
        r = p.send(y)
        total += r
print(total)


def part2():
    def inBeam100(c):
        for point in c, c + 99, c + complex(0, 99):
            p = program(data)
            next(p)
            p.send(point.real)
            if p.send(point.imag) == 0:
                return False
        return True

    def foundAtD(d):
        for x in range(d):
            if inBeam100(complex(x, d - x)):
                return x * 10000 + d - x
        return None

    d = 1000
    while foundAtD(d) == None:
        d += 1000

    def findSmallest(start, end):
        if end - start < 2:
            print(foundAtD(end))
        else:
            mid = start + (end - start) // 2
            if foundAtD(mid) != None:
                findSmallest(start, mid)
            else:
                findSmallest(mid, end)

    findSmallest(d - 1000, d)


part2()
